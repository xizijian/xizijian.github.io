...
layout:     post
title:      "iOS浅谈性能优化"
subtitle:   "iOS Introduction to performance optimization"
date:       2017-08-17
author:     "Mr.Xi"
...
App性能问题是一直存在的，特别是在国内。大部分人并没有意识到性能问题的重要性，没有意识到当用户流失是性能问题导致的。众所周知，iPhone上面的应用一直都是以流畅的操作体验著称，但是由于之前开发人员把注意力更多的放在开发功能上面，比较少去考虑性能的问题，这就导致页面之间的切换没有那种很流畅的感觉，内存也居高不下，比较影响应用的用户体验，所以很有必要进行一些优化。
...
假如最上层的view是不透明的，那直接使用这个view的对应颜色值就可以，但如果是透明的，在计算像素的颜色值时就需要计算它下面的图层，透明的试图越多，计算量就越大，因此也会对图形的性能产生一定的影响，所以可以的话也尽量减少透明图层的数目。


### 显示图片大小尽量跟ImageView大小保持一致

*如果要在UIImageView中显示一个来自bundle的图片，应保证图片的大小和UIImageView的大小相同。在运行中缩放图片是很耗费资源的。
* 如果图片是从远端服务加载的你不能控制图片大小，比如在下载前调整到合适大小的话，可以在下载完成后，最好是用**background thread**,缩放一次，然后在UIImageView中使用缩放后的图片。

### 不要在viewWillAppear 中做费时的操作

*  **viewWillAppear**:在view显示之前被调用，出于效率考虑，在这个方法中不要处理复杂费时的事情；适合在此方法中设置view的显示属性之类的简单事情，比如背景色，字体等。要不然，用户会明显感觉到view显示迟钝。

    ![viewWillAppear](img/post-bg-miui-ux.jpg)
